...
#'
#' Evaluates the empirical power and computation time of a two-sample
#' proportion test across different true proportion values for group 2.
#'
#' @param n Sample size for each group
#' @param theta2_range Vector of true proportion values for group 2
#' @param theta1 Fixed true proportion for group 1
#' @param num_simulations Number of Monte Carlo simulations per theta value
#' @param alpha Significance level for empirical power calculation (default = 0.05)
#'
#' @return A data frame with columns:
#'   - theta: True proportion values for group 2
#'   - empirical_power: Estimated empirical power
...
simulate_test <- function(n, theta2_range, theta1, num_simulations, alpha=0.05) {
...
theta2_values <- seq(0.2, 0.9, by = 0.1)  # Varying proportions for group 2

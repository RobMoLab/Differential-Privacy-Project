...
# FIMA is applied to the three cell probabilities needed to recover the main
# effects: (X1, X2) = (0,0), (1,0), and (0,1). The script also reports the
# non-private MLE confidence intervals as a benchmark.
...
# Non-private MLE benchmark confidence intervals. Returns NA if glm/confint fails.
mle_logistic_ci <- function(Y, X1, X2) {
  fit <- tryCatch(glm(Y ~ X1 + X2, family = binomial()), error = function(e) NULL)
  if (is.null(fit)) return(matrix(NA_real_, nrow = 3, ncol = 2))

  ci <- tryCatch(suppressMessages(confint(fit)), error = function(e) NULL)
  if (is.null(ci)) return(matrix(NA_real_, nrow = 3, ncol = 2))
...
run_simulation <- function(
    ...
    sample_sizes = c(30, 100, 200, 500, 1000, 2000),
    alpha = 0.05,
    H = 10^4,
    B = 10^4,
  ...
  beta0_coverage <- matrix(NA_real_, length(sample_sizes), length(eps_values),
                           dimnames = list(sample_sizes, as.character(eps_values)))
  ...
  # MLE benchmark does not depend on eps, so it is stored once per sample size.
  mle_coverage <- matrix(NA_real_, length(sample_sizes), length(beta),
                         dimnames = list(sample_sizes, names(beta)))
  mle_CI_length <- mle_coverage
  ...
  for (e in seq_along(eps_values)) {
    ...
    for (m in seq_along(sample_sizes)) {
      ...
      mle_cover_b <- matrix(NA, B, length(beta))
      mle_length_b <- matrix(NA_real_, B, length(beta))
      ...
      for (b in seq_len(B)) {
        ...
        # Compute MLE benchmark only once for each sample size, because it does not
        # depend on eps. This condition avoids repeated work across eps values.
        if (e == 1) {
          ci_mle <- mle_logistic_ci(Y, X1, X2)
          mle_length_b[b, ] <- ci_mle[, 2] - ci_mle[, 1]
          mle_cover_b[b, ] <- ci_mle[, 1] < beta & beta < ci_mle[, 2]
        }
      }

      beta0_coverage[m, e] <- mean(coverage[, 1], na.rm = TRUE)
      beta1_coverage[m, e] <- mean(coverage[, 2], na.rm = TRUE)
      beta2_coverage[m, e] <- mean(coverage[, 3], na.rm = TRUE)
      beta0_CI_length[m, e] <- mean(ci_length[, 1], na.rm = TRUE)
      beta1_CI_length[m, e] <- mean(ci_length[, 2], na.rm = TRUE)
      beta2_CI_length[m, e] <- mean(ci_length[, 3], na.rm = TRUE)

      if (e == 1) {
        mle_coverage[m, ] <- colMeans(mle_cover_b, na.rm = TRUE)
        mle_CI_length[m, ] <- colMeans(mle_length_b, na.rm = TRUE)
      }
    }
  }

  list(
    beta0_coverage = cbind(beta0_coverage, MLE = mle_coverage[, "beta0"]),
    beta1_coverage = cbind(beta1_coverage, MLE = mle_coverage[, "beta1"]),
    beta2_coverage = cbind(beta2_coverage, MLE = mle_coverage[, "beta2"]),
    beta0_CI_length = cbind(beta0_CI_length, MLE = mle_CI_length[, "beta0"]),
    beta1_CI_length = cbind(beta1_CI_length, MLE = mle_CI_length[, "beta1"]),
    beta2_CI_length = cbind(beta2_CI_length, MLE = mle_CI_length[, "beta2"])
  )
}

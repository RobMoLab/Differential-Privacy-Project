...
(p_val <- (sum(dist > 0) + 1)/(H + 1)) # HA: p2 > p1
...
dp_pi <- c(dp_prop(pi1, eps = eps, n = n1),
           dp_prop(pi2, eps = eps, n = n2),
           dp_prop(pi3, eps = eps, n = n3))
           #dp_prop(pi4, eps = eps, n = n4))

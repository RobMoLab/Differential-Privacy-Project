...
# Distribution of Difference of two sample JINI 
...
#start_time <- Sys.time()
        ...
        # H0: theta1-theta2 = 0      Vs    H1: theta1-theta2 < 0
...
for (j in 1:length(theta0)){
 ...
 # plot(theta0, level_res_PJ2 , type = "b", pch = 19, col = "blue", xlab = "theta values", ylab = "levels", ylim = c(0, 1), main = "Level of Two Samples Private JINI Test (H=B=10^3, eps=1,alpha=0.05, n1=30, n2=40)")
 # abline(h = 0.05, col = "purple")  
 
 
 #To run on HPC
 # Plot of level
 pdf("level_Two_sample_Private_JINI.pdf")  # Save plot as PDF on Auburn HPC
 plot(theta0, level_res_PJ2 , type = "b", pch = 19, col = "blue", xlab = "theta values", ylab = "levels", ylim = c(0, 1), main = "Level of Two Samples Private JINI Test (H=B=10^3, eps=1,alpha=0.05, n1=30, n2=40)")
 abline(h = 0.05, col = "purple")  
 dev.off()  
